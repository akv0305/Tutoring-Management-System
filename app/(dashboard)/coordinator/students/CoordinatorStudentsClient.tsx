...
function ActionButtons({ row }: { row: Student }) {
  ...
  return (
    <div className="flex items-center gap-1">
      ...
      {type === "payment" && (
        <button title="Confirm Payment" className="p-1.5 rounded-md text-[#F59E0B] hover:bg-amber-50 transition-colors">
          <CreditCard className="w-3.5 h-3.5" />
        </button>
      )}
      {type === "package" && (
        <button title="Assign Package" className="p-1.5 rounded-md text-[#0D9488] hover:bg-teal-50 transition-colors">
          <Package className="w-3.5 h-3.5" />
        </button>
      )}
      {type === "alert" && (
        <button title="Follow Up" className="p-1.5 rounded-md text-[#EF4444] hover:bg-red-50 transition-colors">
...
const columns = [
  ...
  },
  {
    key: "actions",
    label: "Actions",
    render: (r: Student) => <ActionButtons row={r} />,
  },
]
